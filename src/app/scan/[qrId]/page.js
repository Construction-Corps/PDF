...
const PublicScanPage = () => {
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          // Allow scanning without location for now
        }
      );
    }
    ...
    const fetchItemDetails = async () => {
        try {
            // Note: This assumes the QR ID is the same as the item's qr_code UUID
            // We fetch all items and find the one with the matching QR code.
